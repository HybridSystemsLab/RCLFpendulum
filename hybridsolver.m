function [t x j] = hybridsolver( f,g,C,D,x0,T,J,rule,options,MaxStep)
% HYBRIDSOLVER solves hybrid equations
%   [t x j] = hybridsolver( f,g,C,D,x0,T,J) will integrate
%   x'=f(x) and jump by the rule x = g(x). y is a vector with the same
...
%
%   MaxStep - set the maximum step length. At each run of the
